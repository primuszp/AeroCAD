...
{
    class ModelSpace : Canvas, IModelSpace, ISelectionService
    {
        ...
        public ModelSpace()
        {
            ...
            this.Background = new SolidColorBrush(Color.FromArgb(255, 33, 40, 48));
        ...
        protected override void OnVisualChildrenChanged(DependencyObject visualAdded, DependencyObject visualRemoved)
        {
            ...
            Canvas.SetZIndex(this.screen, this.Children.Count);
        }
        ...
        public void ClearSelection()
        {
            if (this.SelectedObjects.Count > 0)
            {
                ...
                {
                    if (child is ISelectionService)
                        (child as ISelectionService).ClearSelection();
                }
            }
        ...
        public bool HitTest(Point point)
        {
            foreach (var child in this.Children)
            {
                if (child is ISelectionService)
                {
                    ISelectionService layer = child as ISelectionService;

                    if (layer != null && layer.HitTest(point))
                    {
                        foreach (ISelectable obj in layer.SelectedObjects)
                        {
                            if (!this.SelectedObjects.Contains(obj)) this.SelectedObjects.Add(obj);
                        }
                        return true;
                    }
                }
        ...
        public bool HitTest(Rect rect)
        {
            ...
            foreach (var child in this.Children)
            {
                if (child is ISelectionService)
                {
                    ...
                    if (layer != null && layer.HitTest(rect))
                    {
                        foreach (ISelectable obj in layer.SelectedObjects)
                        {
