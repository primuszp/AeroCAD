...
{
    public class CircleTool : InteractiveCommandToolBase, IMouseListener, IKeyboardListener, ILayerBoundTool
    {
        private readonly CircleCommandController controller;

        public CircleTool() : base("CircleTool")
        {
            controller = new CircleCommandController(() => ActiveLayer ?? GetFirstLayer());
        }
        ...
        public override bool TrySubmitToken(Editor.CommandInputToken token)
        {
        ...
        {
            return ApplyResult(controller.TrySubmitToken(this, Editor.CommandInputToken.Point(FormatPoint(point), point)));
        }
